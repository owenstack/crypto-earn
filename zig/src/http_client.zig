...
pub const HttpClient = struct {
    ...
    pub fn deleteJsonWithHeaders(self: *HttpClient, url: []const u8, json_body: []const u8, headers: []const std.http.Header) HttpError!Response {
        const uri = std.Uri.parse(url) catch {
            log.err("http", "invalid url: {s}", .{url});
            return error.InvalidUrl;
        };
        ...
        const result = self.client.fetch(.{
            ...
            log.err("http", "DELETE {s} failed", .{url});
            return error.RequestFailed;
        };

        const status_class = result.status.class();
        if (status_class == .server_error) {
            log.err("http", "DELETE {s} -> {d}", .{ url, @intFromEnum(result.status) });
            return error.ServerError;
        }
        if (status_class == .client_error) {
            log.warn("http", "DELETE {s} -> {d}", .{ url, @intFromEnum(result.status) });
            return error.ClientError;
        }
